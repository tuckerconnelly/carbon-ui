...
opacityAV=new _reactNativeUniversal.Animated.Value(_this.props.children?1:0),_temp),_possibleConstructorReturn(_this,_ret);}// Using state.text to delay the removal of the text so it.
// can animate/fade out.
_createClass(Error,[{key:'componentWillReceiveProps',value:function componentWillReceiveProps(nextProps){if(!this.props.children&&nextProps.children)return this.show(nextProps.children);if(this.props.children&&!nextProps.children)return this.hide();if(this.props.children!==nextProps.children)return this.changeTo(nextProps.children);}},{key:'show',value:function show(text){
...
_styles.Animations.standard(_this3.opacityAV,1,150).start();
});
...
{var _props=
this.props;var style=_props.style;var theme=_props.theme;var other=_objectWithoutProperties(_props,['style','theme']);
