...
_openAV=new _reactNativeUniversal.Animated.Value(_this.props.open?1:0),_temp),_possibleConstructorReturn(_this,_ret);}_createClass(NavigationDrawer,[{key:'componentWillReceiveProps',value:function componentWillReceiveProps(next){var _this2=this;var _props,open,onFinishOpening,onStartClosing;return regeneratorRuntime.async(function componentWillReceiveProps$(_context){while(1){switch(_context.prev=_context.next){case 0:_props=this.props,open=_props.open,onFinishOpening=_props.onFinishOpening,onStartClosing=_props.onStartClosing;if(!open&&next.open){this.setState({shown:true},function(){_index.Animations.entrance(_this2._openAV,{useNativeDriver:true}).start(onFinishOpening);});}if(!(open&&!next.open)){_context.next=6;break;}_context.next=5;return regeneratorRuntime.awrap(onStartClosing());case 5:_index.Animations.exit(this._openAV,{toValue:0,useNativeDriver:true}).start(function(){return _this2.setState({shown:false});});case 6:case'end':return _context.stop();}}},null,this);}},{key:'render',value:function render()

{var _props2=
this.props,onOverlayPress=_props2.onOverlayPress,children=_props2.children;
...
_react2.default.createElement(_reactNativeUniversal.Animated.View,{
css:styles.menu,
style:(0,_uranium.animate)(styles.menuClosed,styles.menuOpen,this._openAV)},
children)));
